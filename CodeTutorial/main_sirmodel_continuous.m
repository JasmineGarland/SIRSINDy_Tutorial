close all; clc; clear;
%% SIR SINDy example with CONTINUOUS model
%% Section 1: Simulate data and use continuous model for dx (i.e. calculate the derivative directly)
...
% Continuous model (i.e. calculate the derivative directly)
for i=1:(length(t))
    dx(i,:) = rhsode(t(i),x(i,:)',pars,b);
end
...
title(sprintf('Raw Data (solid) vs SINDY Model (dotted)\nlambda = %f,  S0 = %f', lambda, S0))%ylim([0 N])
hold off

%% Section 3: Sensitivity to initial condition
% Same for loop as in section 3, looping over S0 values instead of lambda
% values and plotting data vs. SINDy
% lambda = 0.0005;
% S0vec = linspace(0.01*N, 0.2*N, 5);
% 
% for kk = 1:length(S0vec)
%     S0 = S0vec(kk);
%     x0 = [S0; 5*10^-5;];
%     
%     Theta = poolDataSeasonal(t,x,n,polyorder, 0, 1, 52, beta_vec);
...
%     plot(tSINDY, xSINDY(:,2), ':', 'color', '#9D2208', 'LineWidth',2)
%     % plot(tSINDY, N - xSINDY(:,1) - xSINDY(:,2), ':', 'color', '#0076a8', 'LineWidth',2)
%     legend('S', 'I', 'S_{SINDy}', 'I_{SINDy}', 'Location', 'East')
%     title(sprintf('Raw Data (solid) vs SINDY Model (dotted)\nlambda = %f,  S0 = %f', lambda, S0))
%     %xlim([700 720])
%     %ylim([0 N])
%     hold off
%     
...
%% Section 4: Sensitivity to lambda
% lambdavec = [0.00001, 0.0001, 0.001, 0.01, 0.1, 1];
% S0 = 0.1*N;
% x0 = [S0; 5*10^-5;];
...
%     plot(tSINDY, N - xSINDY(:,1) - xSINDY(:,2), ':', 'color', '#0076a8', 'LineWidth',2)
%     legend('S', 'I', 'S_{SINDy}', 'I_{SINDy}', 'Location', 'East')
%     title(sprintf('Raw Data (solid) vs SINDY Model (dotted)\nlambda = %f,  S0 = %f', lambda, S0))    %xlim([700 720])
%     ylim([0 N])
%     hold off
% end
